...
export default {
  ...
  async asyncData(context) {
    let [getDataRes, getListRes] = await Promise.all([
      context.store.dispatch("routes/getItem", context),
  ...
  methods: {
    ...
    update(data) {
      var thus = this;
      thus.$store
        .dispatch("routes/updateItem", data)
        .then(function(response) {
          ...
          if (typeof thus.$router.currentRoute.params.id != "undefined") {
            // return thus.$store
            //   .dispatch("routes/getItem", thus.$router.currentRoute)
            //   .then(function(data) {
            ...
            thus.$router.push({
              path: "/admin/route/" + response.data.data.id
            });
          }
        })
    ...
    acceptDelete(evt) {
      ...
      this.$store
        .dispatch("routes/deleteItem", this.item.id)
        .then(res => {
          this.$router.push({
            path: "/admin/routes/"
          });
