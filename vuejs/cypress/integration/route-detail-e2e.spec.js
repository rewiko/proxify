...
describe('Routes Page', function () {
  ...
  sizes.forEach((size) => {
    it(`should be able to create, update and delete route - viewport ${size}`, function () {
      defineViewPort(size)
      ...
      var payload = {
        "meta": { "count": 1 }, "data": {
          ...
          "path": "cy-mock" + uuid_name, "method": "cy-mock" + uuid_name, "updatedAt": "2019-01-20T10:33:02.642Z", "createdAt": "2019-01-20T10:33:02.642Z"
        }
      }
      var payloadPut = {
        "meta": { "count": 1 }, "data": {
          "active": true, "id": idGenerated,
          "path": "cy-mock-update" + uuid_name, "method": "cy-mock" + uuid_name, "updatedAt": "2019-01-20T10:33:02.642Z", "createdAt": "2019-01-20T10:33:02.642Z"
        }
      }
      cy.route({
        method: 'POST',
      ...
      cy.get('[data-cy="pathField"]').type("cy-mock" + uuid_name)
      cy.get('[data-cy="methodField"]').type("cy-mock" + uuid_name)
      ...
      testSidebar(size);
      cy.url().should('include', '/admin/route/' + idGenerated)
      ...
      cy.get('[data-cy="pathField"]').then((element) => {
        expect(element.val()).to.be.equal("cy-mock" + uuid_name)
      });
      cy.get('[data-cy="methodField"]').then((element) => {
        expect(element.val()).to.be.equal("cy-mock" + uuid_name)
      });

      cy.get('[data-cy="pathField"]').type("cy-mock-update" + uuid_name)
      cy.get('[data-cy="submitForm"]').click()
